...
export default function ProfilePage() {
  ...
  const fetchProfile = useCallback(() => {
    ...
    const p = getWorkerProfile(workerId);
    if (p) { setProfile(p); } else { router.push('/profile/setup'); }
    setLoading(false);
  }, [router]);
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950">
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-6">
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6">
          ...
          <div className="mt-4 pt-4 border-t border-zinc-100 dark:border-zinc-800">
            <p className="text-xs text-zinc-500">
              {profile.interaction_level === 'peer' && <>Your experience is recognised. CoAssure will keep prompts brief and focus on what&rsquo;s specific to the job today.</>}
              {profile.interaction_level === 'standard' && <>CoAssure adapts to your experience. You&rsquo;ll get focused safety prompts tailored to each job.</>}
              {profile.interaction_level === 'supportive' && <>CoAssure provides extra context as you build your experience. The more you use it, the more it adapts.</>}
            </p>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-5">
          <TrainingList records={profile.training_records} workerId={profile.id} onUpdate={fetchProfile} />
        </div>
